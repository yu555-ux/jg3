          ...
          {multiApiEnabled && (
            <div className="space-y-3">
              ...
              <div className="p-4 bg-white/30 border border-emerald-400/90 rounded-2xl space-y-3 backdrop-blur-md">
                ...
                {!presetOpen && null}
                {presetOpen && (
                  <div className="mt-2 p-3 rounded-xl border border-emerald-400/90 bg-white/60 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-[10px] text-slate-500">
                        <span>共 {presetRules.length} 条规则 / 启用 {enabledCount}</span>
                        <button
                          onClick={refreshWorldbookOptions}
                          className="px-2 py-1 rounded-lg border border-emerald-400/90 bg-white/70 text-emerald-700 hover:bg-emerald-50 transition-colors"
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-2 py-1 text-[10px] font-bold rounded-lg border border-emerald-400/90 bg-white/70 text-emerald-700 hover:bg-emerald-50 transition-colors"
                      >
                        导入预设
                      </button>
                      <button
                    ...
                    <div className="space-y-3">
                      ...
                      {Object.entries(groupedRules).map(([groupName, rules]) => {
                        ...
                        return (
                          <div key={groupName} className="rounded-xl border border-emerald-300/70 bg-white/70">
                            <button
                              type="button"
                              onClick={() => setPresetGroupOpen(prev => ({ ...prev, [groupName]: !groupOpen }))}
                              className="w-full px-3 py-2 text-[11px] font-black text-emerald-700 uppercase tracking-widest border-b border-emerald-300/70 flex items-center justify-between"
                            >
                                  ...
                                  return (
                                    <div key={rule.id || index} className="p-3 space-y-2">
                                      <div className="flex items-center gap-2">
                                        ...
                                        <button
                                          type="button"
                                          className="ml-1 inline-flex items-center justify-center w-7 h-7 rounded-full border border-emerald-300/70 bg-white/70 text-emerald-700 hover:bg-emerald-50 transition-colors"
                                          title="引用世界书"
                                          onClick={() => {
                                      ...
                                      {worldbookPanelOpen[index] && (
                                        <div className="rounded-xl border border-emerald-300/70 bg-white/70 p-2 space-y-2">
                                          <div className="flex items-center gap-2">
                                            <button
                                              type="button"
                                              className="px-3 py-1.5 text-xs font-bold rounded-lg border border-emerald-300/70 bg-white/70 text-emerald-700 hover:bg-emerald-50 transition-colors"
                                              onClick={() => setWorldbookPickerOpen(prev => ({ ...prev, [index]: !prev[index] }))}
                                            >
                                              插入世界书
                                            </button>
                                            <div className="flex-1 px-2.5 py-1.5 text-xs rounded-lg border border-emerald-300/70 bg-white/80 text-slate-400">
                                              点击选择世界书条目...
                                            </div>
                                          </div>
                                          {worldbookPickerOpen[index] && (
                                            <div className="mt-2 p-2 rounded-lg border border-emerald-300/70 bg-white/80 space-y-2">
                                              <input
                                                className="w-full px-2.5 py-1.5 text-xs rounded-lg border border-emerald-300/70 bg-white"
                                                placeholder="搜索世界书条目..."
                                                value={worldbookSearch[index] || ''}
                                                onChange={e => setWorldbookSearch(prev => ({ ...prev, [index]: e.target.value }))}
                                              />
                                              <div className="max-h-40 overflow-y-auto rounded-lg border border-emerald-300/70 bg-white">
                                                {(worldbookOptions || [])
                                                      ...
                                                      >
                                                        <span
                                                          className={`w-3.5 h-3.5 rounded border ${
                                                            selected ? 'bg-emerald-500 border-emerald-500' : 'border-emerald-300/70'
                                                          }`}
                                                        />
                                                        {name}
                                                      </button>
                                      ...
                                      <textarea
                                        className="w-full min-h-[120px] px-2.5 py-2 text-xs rounded-xl border border-emerald-300/70 bg-white/80 focus:outline-none focus:ring-2 focus:ring-emerald-200"
                                        value={rule.content || ''}
